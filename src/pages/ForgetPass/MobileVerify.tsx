...
const MobileVerify = () => {
  ...
  return (
    <form
      className="flex h-screen w-full text-white flex-col bg-gradient-to-r from-[#3A7D44] to-[#172533] justify-center items-center overflow-hidden"
      onSubmit={(e) => {
        e.preventDefault();
        HandleVerify();
      }}
    >
      <AnimatePresence mode="sync">
        ...
        <div className="flex w-fit h-fit ltr mt-20 justify-center">
          ...
          >
            ...
            <OtpInput
              ...
              numInputs={5}
              containerStyle={
                "w-full md:mt-5 sm:mt-5 mt-7 justify-center items-center flex"
              }
              inputType="tel"
              inputStyle={
                "flex md:h-[39px] sm:h-[39px] h-[30px] md:scale-139 sm:scale-135 scale-200 font-[vazirmatn] font-normal md:text-[28px] sm:text-[28px] text-[24px] text-black text-center bg-gray-300 rounded-[18px] border-2 border-gray-300 transition-all ease-in-out duration-300 shadow-md focus:outline-none focus:border-blue-500 focus:bg-white focus:shadow-lg md:mx-[15px] sm:mx-[15px] mx-[17px]"
              }
        ...
        <div
          className={`flex w-fit h-fit flex-row md:mt-8 sm:mt-8 mt-10 justify-center ${timeLeft != 0 ? "" : "ring-2 p-2 rounded-2xl"}`}
        >
          <motion.div
            key="verify3"
            initial={{ opacity: 0, x: -50 }}
          ...
          >
            <div className="flex flex-row w-fit h-fit mx-4">
              {timeLeft != 0 ? (
                <p
                  className={`w-fit text-right md:text-[20px] sm:text-[18px] text-[16px] text-white font-[vazirmatn] font-normal`}
                >
                  دریافت مجدد کد
                </p>
              ) : (
        ...
        <div className="flex w-fit h-fit my-3">
          ...
          >
            <button
              type="submit"
              className={`w-full transition duration-200 ease-in-out rounded-[20px] mt-5 bg-[#3A7D44] shadow-[0_4px_10px_rgba(0,0,0,0.2)] py-3 ${!(token.length === 5) ? "opacity-60" : "hover:bg-green-600  cursor-pointer "}`}
